...
class SqlDatabaseOptimized {
  ...
  async initialize(): Promise<void> {
    ...
    this.db.run(`CREATE TABLE IF NOT EXISTS erc_20_transfer_events (
      ...
      FOREIGN KEY (transaction_hash) REFERENCES transactions (transaction_hash) ON UPDATE CASCADE ON DELETE CASCADE,
  ...
  addToken(token: TokenContract) {
    ...
    this.db.run(
      `INSERT OR IGNORE INTO contracts(contract_id, address_id, deployer_id, block_number, timestamp, type)
      VALUES (
  ...
  async addTransaction(tx: SimplifiedTransaction): Promise<number> {
    return new Promise((res, rej) => {
      ...
      this.db.run(
        `INSERT INTO transactions(transaction_hash, sighash, from_id, to_id, block_number, tx_index, timestamp) 
            SELECT 
              $hash, 
              $sighash, 
  ...
  addErc20ApprovalEvent(event: TokenInsertEvent<DetailedErc20ApprovalEvent>) {
    ...
    return this.run(
      `
        INSERT OR IGNORE INTO erc_20_approval_events(transaction_hash, log_index, contract_id, owner_id, spender_id, value)
        VALUES ( 
            $transaction_hash, 
            $log_index,
  ...
  addErc20TransferEvent(event: TokenInsertEvent<DetailedErc20TransferEvent>) {
    ...
    return this.run(
      `
        INSERT OR IGNORE INTO erc_20_transfer_events(transaction_hash, log_index, contract_id, from_id, to_id, value)
        VALUES ( 
            $transaction_hash, 
            $log_index,
            (SELECT address_id from addresses WHERE address = $contract), 
  ...
  addErc721ApprovalEvent(event: TokenInsertEvent<DetailedErc721ApprovalEvent>) {
    ...
    return this.run(
      `
        INSERT OR IGNORE INTO erc_721_approval_events(transaction_hash, log_index, contract_id, owner_id, approved_id, token_id)
        VALUES ( 
            $transaction_hash, 
            $log_index,
            (SELECT address_id from addresses WHERE address = $contract), 
  ...
  addErc721TransferEvent(event: TokenInsertEvent<DetailedErc721TransferEvent>) {
    ...
    return this.run(
      `
        INSERT OR IGNORE INTO erc_721_transfer_events(transaction_hash, log_index, contract_id, from_id, to_id, token_id)
        VALUES ( 
            $transaction_hash,
            $log_index,
            (SELECT address_id from addresses WHERE address = $contract), 
  ...
  addErc721ApprovalForAllEvent(event: TokenInsertEvent<DetailedErc721ApprovalForAllEvent>) {
    ...
    return this.run(
      `
        INSERT OR IGNORE INTO erc_721_approval_for_all_events(transaction_hash, log_index, contract_id, owner_id, operator_id, approved)
        VALUES ( 
            $transaction_hash,
            $log_index,
            (SELECT address_id from addresses WHERE address = $contract), 
  ...
  addErc1155ApprovalForAllEvent(event: TokenInsertEvent<DetailedErc1155ApprovalForAllEvent>) {
    ...
    return this.run(
      `
        INSERT OR IGNORE INTO erc_1155_approval_for_all_events(transaction_hash, log_index, contract_id, owner_id, operator_id, approved)
        VALUES ( 
            $transaction_hash,
            $log_index,
            (SELECT address_id from addresses WHERE address = $contract), 
  ...
  addErc1155TransferSingleEvent(event: TokenInsertEvent<DetailedErc1155TransferSingleEvent>) {
    ...
    return this.run(
      `
        INSERT OR IGNORE INTO erc_1155_transfer_single_events(transaction_hash, log_index, contract_id, operator_id, from_id, to_id, token_id, value)
        VALUES ( 
            $transaction_hash,
            $log_index,
            (SELECT address_id from addresses WHERE address = $contract), 
...
export default SqlDatabaseOptimized;
